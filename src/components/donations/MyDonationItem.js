...
function MyDonationItem({ donation }) {
  ...
  return (
    <motion.div className="flex flex-col rounded-xl border border-gray-100   justify-center content-center bg-gray-50 min-h-[300px]">
      {/* image section */}
        ...
        {options && (
          ...
          >
            <div className="w-full  rounded-full p-1 flex flex-row justify-end cursor-pointer">
              <BsX
                onClick={() => setOptions(!options)}
                className="text-md bg-gray-600 text-white rounded-full"
              />
            </div>
            ...
            <h1
              onClick={() =>
                navigate(`/account/donations/${donation._id}/edit`)
              }
      ...
      <div className="  flex flex-col items-start justify-center content-center p-2  bg-gray-50 rounded-xl">
        ...
        <p className="text-sm text-left max-w-sm  text-gray-600  p-1 ">
          {description}
        </p>
      </div>
    </motion.div>
